...
void AfterSimInit(void){
	...
	#endif
	if (0/*OK*/ != A661ConnectServer("127.0.0.1", 1231, " \"C:/Dev/ces-65/socket_test/KCG/test_UA_1.bin\"", "C:/Dev/ces-65/socket_test/KCG/test_UA_1_conf.xml" )) {
		const char *pszError;
...
void BeforeSimStep(void){
	...
	/* reveive from server */
	test_UA_1_receive_clear(NULL, NULL);
	if (A661Receive(msg, sizeof msg) == 0)
		test_UA_1_receive(msg, sizeof msg, NULL, NULL);
}
