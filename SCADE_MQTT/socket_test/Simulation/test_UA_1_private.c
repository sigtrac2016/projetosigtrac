...
/* Widget SelectionListButton for parameter A661_OPENING_ENTRY */
 uaa_size a661_com_msg_short(buffer_el msg[], 
  uaa_uint16 pid,  kcg_int A661_OPENING_ENTRY_OpeningEntry_){
  WRITE_USHORT(msg, pid);
  WRITE_USHORT(msg + 2, A661_OPENING_ENTRY_OpeningEntry_);
  return 4 ;
}

/* Pack messages for widget SelectionListButton: A661_OPENING_ENTRY */
 uaa_size a661_com_csp_widget_SelectionListButton(buffer_el msg[], 
  uaa_uint16 wid,  uaa_bool A661_OPENING_ENTRY__emit__, 
  kcg_int A661_OPENING_ENTRY_OpeningEntry_){
  uaa_size offset = 0;
  if (A661_OPENING_ENTRY__emit__){
    offset = 8;
    a661_msg_csp_header(msg, wid);
    if (A661_OPENING_ENTRY__emit__){
      offset = offset + a661_com_msg_short(msg + offset, A661_OPENING_ENTRY,
        A661_OPENING_ENTRY_OpeningEntry_);
    }
